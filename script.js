...
function initHeroGlassLens() {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Magnification factor (1.45x zoom)
        float zoom = 1.45;

        // Apply magnification zoom inside the lens, with slightly offset factors
        // for a beautiful glass refraction / chromatic aberration effect
        vec2 uvRed = mix(uv, uMouse + (uv - uMouse) / (zoom - 0.07), mask);
        vec2 uvGreen = mix(uv, uMouse + (uv - uMouse) / zoom, mask);
        vec2 uvBlue = mix(uv, uMouse + (uv - uMouse) / (zoom + 0.07), mask);

        vec4 redSample = texture2D(uTexture, uvRed);
        vec4 greenSample = texture2D(uTexture, uvGreen);
        vec4 blueSample = texture2D(uTexture, uvBlue);
  ...
  function drawText() {
    ...
    ctx.textAlign = "left";
    ctx.font = `600 ${headlineSize}px "Bricolage Grotesque", sans-serif`;
    ctx.fillText("Cinematic", pad, top);

    ctx.fillStyle = "#f5b027";
    ctx.textAlign = "right";
    ctx.font = `italic 600 ${scriptSize}px "Instrument Serif", Georgia, serif`;
...
function initAboutHeroEntrance() {
  ...
  const chars = aboutHero.querySelectorAll(".about-hero__title .text-roll__char");
...
/* ----- Oliver Parallax Gallery: scroll-driven columns ----- */
